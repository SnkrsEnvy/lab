...
(()=>{
'use strict';
const BUILD='PS-PUBLIC-DEMO-G6S-v020';
...
function renderPage(){
  ...
  if(S.doc.kind==='pdf'&&S.pdfInspection){
    ...
    if(meta){
      ...
      S.pdfStaged.filter(op=>op.type==='insert_image'&&Number(op.page)===pageNo).forEach(op=>{
        const target=(op.bbox||[]).map(Number);if(target.some(Number.isNaN))return;
        const el=document.createElement('div');el.title='Staged image insertion';
        Object.assign(el.style,{position:'absolute',left:(target[0]/meta.width*100)+'%',top:(target[1]/meta.height*100)+'%',width:((target[2]-target[0])/meta.width*100)+'%',height:((target[3]-target[1])/meta.height*100)+'%',border:'2px solid #315f4b',background:'rgba(49,95,75,.14)',pointerEvents:'none',zIndex:84});
        el.innerHTML='<span style="position:absolute;left:3px;top:3px;background:#315f4b;color:white;font-size:8px;padding:2px 4px">ADD IMAGE</span>';
        layer.appendChild(el);
      });
      S.pdfStaged.filter(op=>op.type==='replace_image'&&Number(op.page)===pageNo).forEach(op=>{
        const source=(op.bbox||[]).map(Number);if(source.some(Number.isNaN))return;
        const el=document.createElement('div');el.title='Staged image replacement';
        Object.assign(el.style,{position:'absolute',left:(source[0]/meta.width*100)+'%',top:(source[1]/meta.height*100)+'%',width:((source[2]-source[0])/meta.width*100)+'%',height:((source[3]-source[1])/meta.height*100)+'%',border:'2px solid #315f4b',background:'rgba(49,95,75,.18)',pointerEvents:'none',zIndex:83});
        el.innerHTML='<span style="position:absolute;left:3px;top:3px;background:#315f4b;color:white;font-size:8px;padding:2px 4px">REPLACE</span>';
        layer.appendChild(el);
      });
      ...
      S.pdfStaged.filter(op=>op.type==='crop_image'&&Number(op.page)===pageNo).forEach(op=>{
        const source=(op.bbox||[]).map(Number),target=(op.targetBbox||[]).map(Number);if([...source,...target].some(Number.isNaN))return;
        const old=document.createElement('div');old.title='Source image bounds · staged crop';
        Object.assign(old.style,{position:'absolute',left:(source[0]/meta.width*100)+'%',top:(source[1]/meta.height*100)+'%',width:((source[2]-source[0])/meta.width*100)+'%',height:((source[3]-source[1])/meta.height*100)+'%',border:'2px dashed #9c3b32',background:'rgba(156,59,50,.08)',pointerEvents:'none',zIndex:82});
        const next=document.createElement('div');next.title='Staged cropped image bounds';
        Object.assign(next.style,{position:'absolute',left:(target[0]/meta.width*100)+'%',top:(target[1]/meta.height*100)+'%',width:((target[2]-target[0])/meta.width*100)+'%',height:((target[3]-target[1])/meta.height*100)+'%',border:'2px solid #315f4b',background:'rgba(49,95,75,.12)',pointerEvents:'none',zIndex:83});
        next.innerHTML='<span style="position:absolute;left:3px;top:3px;background:#315f4b;color:white;font-size:8px;padding:2px 4px">CROP</span>';
...
function openPdfImageMovePicker(){
  const pageNo=Number(curPage()?.pdfPage||1),meta=S.pdfInspection?.pages?.find(x=>Number(x.page)===pageNo),images=(meta?.images||[]);
  if(!images.length){modal('Move PDF image','<div class="proof-item"><strong class="hold">NO RASTER IMAGES</strong><small>G6M found no embedded raster image occurrences on this page.</small></div>');return;}
  const rows=images.map(img=>{
    const cls=img.movable?'pdf-block-choice':'pdf-block-choice disabled-tool';
    const reason=img.movable?'Unique axis-aligned raster · '+img.bbox.map(v=>Number(v).toFixed(1)).join(', '):'HOLD · '+(img.holdReason||'outside G6M');
  ...
  modal('Move existing PDF image','<p class="pdf-tool-note">Choose one inspected image. G6M translates the same raster on the same page without resizing. Enter X/Y offsets in PDF points; negative values move left/up.</p><div class="pdf-block-list">'+rows+'</div>');
  qa('.pdf-block-choice[data-image]').forEach(btn=>btn.onclick=async()=>{
    ...
    const dxRaw=prompt('Horizontal move in PDF points (positive = right):','36');if(dxRaw==null)return;
    const dyRaw=prompt('Vertical move in PDF points (positive = down):','24');if(dyRaw==null)return;
    const dx=Number(dxRaw),dy=Number(dyRaw);if(!Number.isFinite(dx)||!Number.isFinite(dy)){toast('Image move requires numeric X/Y offsets.');return;}
    const [x0,y0,x1,y1]=img.bbox.map(Number),target=[x0+dx,y0+dy,x1+dx,y1+dy];
    if(target[0]<0||target[1]<0||target[2]>Number(meta.width)||target[3]>Number(meta.height)){modal('PDF image move held','<div class="proof-item"><strong class="hold">TARGET OUTSIDE PAGE</strong><small>The translated image must remain fully inside the current PDF page.</small></div>');return;}
    try{
      const b=pdfBridge();const result=await b.invoke('moveImage',{page:pageNo,bbox:img.bbox,targetBbox:target,xref:img.xref,digest:img.digest});
      S.pdfStaged.push(result.operation);record('Stage bounded PDF image move');q('#modalBg').classList.remove('open');renderAll();toast('PDF image move staged · Undo removes it before export.');
      window.dispatchEvent(new CustomEvent('publisherstudio:pdfcommand',{detail:{tool:'moveImage',result}}));
    }catch(error){modal('PDF image move failed','<div class="proof-item"><strong class="hold">MOVE IMAGE · FAIL</strong><small>'+esc(error?.message||error)+'</small></div>');}
  });
}
...
function readFileDataUrl(file){return new Promise((resolve,reject)=>{const r=new FileReader();r.onerror=()=>reject(new Error('Could not read replacement image.'));r.onload=()=>resolve(String(r.result||''));r.readAsDataURL(file);});}
function choosePdfReplacementFile(){return new Promise(resolve=>{const input=document.createElement('input');input.type='file';input.accept='image/png,image/jpeg';input.hidden=true;document.body.appendChild(input);input.addEventListener('change',()=>{const f=input.files?.[0]||null;input.remove();resolve(f);},{once:true});input.click();});}
function imageDimensionsFromDataUrl(dataUrl){return new Promise((resolve,reject)=>{const im=new Image();im.onload=()=>resolve({width:im.naturalWidth||im.width,height:im.naturalHeight||im.height});im.onerror=()=>reject(new Error('Could not decode inserted image.'));im.src=dataUrl;});}
async function openPdfImageInsertPicker(){
  const pageNo=Number(curPage()?.pdfPage||1),meta=S.pdfInspection?.pages?.find(x=>Number(x.page)===pageNo);
  if(!meta){modal('Add PDF image','<div class="proof-item"><strong class="hold">NO SOURCE PAGE</strong><small>G6R requires a currently inspected PDF source page.</small></div>');return;}
  const file=await choosePdfReplacementFile();if(!file)return;
  if(!['image/png','image/jpeg'].includes(String(file.type||'').toLowerCase())){toast('Inserted image must be PNG or JPEG.');return;}
  if(file.size<1||file.size>750000){toast('Inserted image must be 750 KB or smaller.');return;}
  try{
    const dataUrl=await readFileDataUrl(file),dims=await imageDimensionsFromDataUrl(dataUrl),comma=dataUrl.indexOf(','),imageBase64=comma>=0?dataUrl.slice(comma+1):'';
    const xRaw=prompt('Image left X in PDF points:','72');if(xRaw==null)return;
    const yRaw=prompt('Image top Y in PDF points:','120');if(yRaw==null)return;
    const widthRaw=prompt('Image width in PDF points (aspect ratio preserved):','144');if(widthRaw==null)return;
    const x=Number(xRaw),y=Number(yRaw),w=Number(widthRaw);
    if(!Number.isFinite(x)||!Number.isFinite(y)||!Number.isFinite(w)||w<1){toast('Image placement requires numeric X, Y and width.');return;}
    const h=w*(Number(dims.height)/Number(dims.width)),bbox=[x,y,x+w,y+h];
    if(x<0||y<0||bbox[2]>Number(meta.width)||bbox[3]>Number(meta.height)){modal('PDF image insertion held','<div class="proof-item"><strong class="hold">TARGET OUTSIDE PAGE</strong><small>The inserted image must remain fully inside the current PDF page.</small></div>');return;}
    const b=pdfBridge();const result=await b.invoke('insertImage',{page:pageNo,bbox,imageBase64,imageMime:file.type,imageName:file.name,imageBytes:file.size});
    S.pdfStaged.push(result.operation);record('Stage bounded PDF image insertion');renderAll();toast('PDF image insertion staged · Undo removes it before export.');
    window.dispatchEvent(new CustomEvent('publisherstudio:pdfcommand',{detail:{tool:'insertImage',result}}));
  }catch(error){modal('PDF image insertion failed','<div class="proof-item"><strong class="hold">ADD IMAGE · FAIL</strong><small>'+esc(error?.message||error)+'</small></div>');}
}
function openPdfImageCropPicker(){
  const pageNo=Number(curPage()?.pdfPage||1),meta=S.pdfInspection?.pages?.find(x=>Number(x.page)===pageNo),images=(meta?.images||[]);
  if(!images.length){modal('Crop PDF image','<div class="proof-item"><strong class="hold">NO RASTER IMAGES</strong><small>G6S found no embedded raster image occurrences on this page.</small></div>');return;}
  const rows=images.map(img=>{
    const cls=img.movable?'pdf-block-choice':'pdf-block-choice disabled-tool';
    const reason=img.movable?'Unique axis-aligned raster · '+img.bbox.map(v=>Number(v).toFixed(1)).join(', '):'HOLD · '+(img.holdReason||'outside G6S');
    return '<button class="'+cls+'" data-image="'+esc(img.id)+'" '+(img.movable?'':'disabled')+'><strong>Raster '+esc(img.id)+'</strong><small>'+esc(reason)+'</small></button>';
  }).join('');
  modal('Crop existing PDF image','<p class="pdf-tool-note">Choose one image, then enter percentages to remove from its left, top, right and bottom edges. Each edge is limited to 45%. The PDF bbox shrinks proportionally so surviving pixels keep their physical scale.</p><div class="pdf-block-list">'+rows+'</div>');
  qa('.pdf-block-choice[data-image]').forEach(btn=>btn.onclick=async()=>{
    const img=images.find(x=>x.id===btn.dataset.image);if(!img?.movable)return;
    const values=[];
    for(const [label,def] of [['Left crop %','10'],['Top crop %','12.5'],['Right crop %','20'],['Bottom crop %','25']]){
      const raw=prompt(label+' (0–45):',def);if(raw==null)return;values.push(Number(raw)/100);
    }
    const [left,top,right,bottom]=values;
    if(values.some(v=>!Number.isFinite(v)||v<0||v>0.45)||left+right>=0.9||top+bottom>=0.9||values.reduce((a,b)=>a+b,0)<=0){toast('Crop edges must be 0–45% and leave meaningful content.');return;}
    const [x0,y0,x1,y1]=img.bbox.map(Number),w=x1-x0,h=y1-y0;
    const target=[x0+w*left,y0+h*top,x1-w*right,y1-h*bottom];
    try{
      const b=pdfBridge();const result=await b.invoke('cropImage',{page:pageNo,bbox:img.bbox,targetBbox:target,xref:img.xref,digest:img.digest,cropLeft:left,cropTop:top,cropRight:right,cropBottom:bottom});
      S.pdfStaged.push(result.operation);record('Stage bounded PDF image crop');q('#modalBg').classList.remove('open');renderAll();toast('PDF image crop staged · Undo restores the uncropped source before export.');
      window.dispatchEvent(new CustomEvent('publisherstudio:pdfcommand',{detail:{tool:'cropImage',result}}));
    }catch(error){modal('PDF image crop failed','<div class="proof-item"><strong class="hold">CROP IMAGE · FAIL</strong><small>'+esc(error?.message||error)+'</small></div>');}
  });
}
function openPdfImageReplacePicker(){
  const pageNo=Number(curPage()?.pdfPage||1),meta=S.pdfInspection?.pages?.find(x=>Number(x.page)===pageNo),images=(meta?.images||[]);
  if(!images.length){modal('Replace PDF image','<div class="proof-item"><strong class="hold">NO RASTER IMAGES</strong><small>G6Q found no embedded raster image occurrences on this page.</small></div>');return;}
  const rows=images.map(img=>{
    const cls=img.movable?'pdf-block-choice':'pdf-block-choice disabled-tool';
    const reason=img.movable?'Unique axis-aligned raster · '+img.bbox.map(v=>Number(v).toFixed(1)).join(', '):'HOLD · '+(img.holdReason||'outside G6Q');
    return '<button class="'+cls+'" data-image="'+esc(img.id)+'" '+(img.movable?'':'disabled')+'><strong>Raster '+esc(img.id)+'</strong><small>'+esc(reason)+'</small></button>';
  }).join('');
  modal('Replace existing PDF image','<p class="pdf-tool-note">Choose one inspected image, then choose an opaque PNG/JPEG up to 750 KB. G6Q preserves the exact existing PDF bbox and stages the replacement bytes only until Undo or verified export.</p><div class="pdf-block-list">'+rows+'</div>');
  qa('.pdf-block-choice[data-image]').forEach(btn=>btn.onclick=async()=>{
    const img=images.find(x=>x.id===btn.dataset.image);if(!img?.movable)return;
    const file=await choosePdfReplacementFile();if(!file)return;
    if(!['image/png','image/jpeg'].includes(String(file.type||'').toLowerCase())){toast('Replacement image must be PNG or JPEG.');return;}
    if(file.size<1||file.size>750000){toast('Replacement image must be 750 KB or smaller.');return;}
    try{
      const dataUrl=await readFileDataUrl(file),comma=dataUrl.indexOf(','),replacementBase64=comma>=0?dataUrl.slice(comma+1):'';
      const b=pdfBridge();const result=await b.invoke('replaceImage',{page:pageNo,bbox:img.bbox,xref:img.xref,digest:img.digest,replacementBase64,replacementMime:file.type,replacementName:file.name,replacementBytes:file.size});
      S.pdfStaged.push(result.operation);record('Stage bounded PDF image replacement');q('#modalBg').classList.remove('open');renderAll();toast('PDF image replacement staged · Undo restores the source before export.');
      window.dispatchEvent(new CustomEvent('publisherstudio:pdfcommand',{detail:{tool:'replaceImage',result}}));
    }catch(error){modal('PDF image replacement failed','<div class="proof-item"><strong class="hold">REPLACE IMAGE · FAIL</strong><small>'+esc(error?.message||error)+'</small></div>');}
  });
}
...
async function runPdfTool(tool){
  ...
  if(tool==='replaceImage'){openPdfImageReplacePicker();return;}
  if(tool==='insertImage'){openPdfImageInsertPicker();return;}
  if(tool==='cropImage'){openPdfImageCropPicker();return;}
  modal('PDF tool boundary','<div class="proof-item"><strong class="hold">'+esc(tool.toUpperCase())+' · NOT PROMOTED</strong><small>The current public transport checkpoint proves bounded Edit Text, true redaction, URI link insertion, translation, resize, 90° rotation, deletion, replacement, insertion, and physical-scale-preserving crop of raster images, and isolated page lifecycle. This tool remains disabled until its interaction and verification path is independently proven.</small></div>');
}
...
capture('#proofBtn',()=>modal('Proof State','<div class="proof-grid"><div class="proof-item"><strong class="pass">G5I kernel · controlled PASS</strong><small>Engineering reference remains frozen</small></div><div class="proof-item"><strong class="pass">G6D native authoring · deepened</strong><small>Shared Page/Flow state + range formatting + styles + tables + find/replace + page breaks</small></div><div class="proof-item"><strong class="pass">Save / reopen path</strong><small>Browser save + portable project snapshot</small></div><div class="proof-item"><strong class="pass">G6S PDF Image Crop · CANDIDATE</strong><small>Inherited bounded PDF editing/page lifecycle and raster operations plus edge cropping of one unique opaque axis-aligned raster. The crop shrinks both source pixels and the PDF bbox proportionally so surviving pixels retain physical scale. Rotated/sheared, masked, reused-XObject, vector and arbitrary-transform crop remain held.</small></div></div>'));
