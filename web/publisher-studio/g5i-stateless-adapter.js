...
function create(options={}){
  ...
  async function invoke({tool,source,payload={}}){const rec=requireSource(source);if(tool==='undo')return undo({source});const operation=operationFor(tool,payload);rec.operations.push(operation);return {status:'STAGED',source,tool,operation:clone(operation),operationCount:rec.operations.length};}
