...
function operationFor(tool,payload={}){
  ...
  if(tool==='replaceImage'){
    ...
    const replacementBase64=String(payload.replacementBase64||''),replacementMime=String(payload.replacementMime||'').toLowerCase();
    const replacementName=String(payload.replacementName||'replacement-image');
    const replacementBytes=Number(payload.replacementBytes||0);
    if(!Number.isInteger(xref)||xref<1||!digest)throw new Error('Image replacement requires one inspected source image occurrence.');
    if(!['image/png','image/jpeg'].includes(replacementMime))throw new Error('Replacement image must be PNG or JPEG.');
    if(!replacementBase64||replacementBytes<1||replacementBytes>750000||replacementBase64.length>1100000)throw new Error('Replacement image exceeds the bounded G6Q payload.');
    return {...base,type:'replace_image',xref,digest,replacementBase64,replacementMime,replacementName,replacementBytes};
  }
  if(tool==='deleteImage'){
    if(!base.bbox||base.bbox.length!==4)throw new Error('Image deletion requires a source bbox.');
    const xref=Number(payload.xref),digest=String(payload.digest||'').trim();
    if(!Number.isInteger(xref)||xref<1||!digest)throw new Error('Image deletion requires one inspected source image occurrence.');
    return {...base,type:'delete_image',xref,digest};
  }
  if(tool==='moveImage'||tool==='resizeImage'||tool==='rotateImage'){
    if(!base.bbox||base.bbox.length!==4)throw new Error('Image transform requires a source bbox.');
    const targetBbox=Array.isArray(payload.targetBbox)?payload.targetBbox.map(Number):null;
    if(!targetBbox||targetBbox.length!==4)throw new Error('Image transform requires a target bbox.');
    const xref=Number(payload.xref),digest=String(payload.digest||'').trim();
    if(!Number.isInteger(xref)||xref<1||!digest)throw new Error('Image transform requires one inspected source image occurrence.');
    const type=tool==='resizeImage'?'resize_image':tool==='rotateImage'?'rotate_image':'move_image';
    return {...base,type,targetBbox,xref,digest,degrees:tool==='rotateImage'?90:0};
  }
